function [PER,Xre] = TeCPSGD_M(A_in,Omega_in,Rank,options,OPTS);
...
Xtrue  = OPTS.Xtrue;
PER    = zeros(1,tensor_dims(end));
Xre    = zeros(tensor_dims);
...
%% Performance Evaluation      
                ...
                idx  = find(~I_mat_Omega);
                X_re = I_mat_Omega;
                X_re(idx) = L_rec(idx);
                er   = L_rec(idx) - Xt_true(idx);
                PER(1,k)   = PER(1,k)  + norm(er)/norm(Xt_true(idx));
                Xre(:,:,k) = X_re;
%%
%             if store_matrix
%                 E_rec = I_mat - L_rec;
