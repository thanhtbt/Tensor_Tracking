function [Factor_Es,PER] = WTucker(X_cell,tucker_rank,OPTS);
...
for t = 1 : T
    ...
    X = double(X);
    ind = known; %find(double(Omiga));
    rest = 1*tucker_rank(1:end-1);
    initial = randn(tensor_dim);
    topt_maxiter = 300;
    poblano_maxiter = 100;
    topt_verbose = true;
    [X_re,Result] = ...
        tucker_opt_track(size(X), X(ind), ind, rest, ...
    ...
    if flag == 1
        ...
        for n = 1 : N-1
            PER(n,t) = sub_est_per(Factor_Es{n,1},Factor_True{t,n},'SEP');
        end
        PER(N+1,t) = t_end;
    else
...
function [X, Result] = tucker_opt_track(s, y, ind, r, varargin)
